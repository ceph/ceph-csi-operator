...
var _ = Describe("ClientProfile Controller", func() {
	Context("When reconciling a resource", func() {
		...
		clientProfile := &csiv1beta1.ClientProfile{}
		typeCephConnectionName := types.NamespacedName{
			Name:      cephConnectionName,
			Namespace: namespaceName,
		}
		cephConn := &csiv1beta1.CephConnection{}

		BeforeEach(func() {
			err := k8sClient.Get(ctx, typeCephConnectionName, cephConn)
			if err != nil && errors.IsNotFound(err) {
				resource := &csiv1beta1.CephConnection{
					ObjectMeta: metav1.ObjectMeta{
						Name:      cephConnectionName,
						Namespace: namespaceName,
					},
					Spec: csiv1beta1.CephConnectionSpec{
			...
			if err != nil && errors.IsNotFound(err) {
				resource := &csiv1beta1.ClientProfile{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					// TODO(user): Specify other spec details if needed.
					Spec: csiv1beta1.ClientProfileSpec{
		...
		AfterEach(func() {
			...
			cephConnection := &csiv1beta1.CephConnection{}

...
func (r *DriverReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !reconcileHandler.snapshotMetadataCertExpiry.IsZero() {
		return ctrl.Result{
			Requeue:      true,
			RequeueAfter: time.Until(reconcileHandler.snapshotMetadataCertExpiry.Add(-24 * time.Hour)), // Requeue 24 hours before expiry
		}, err
	}
...
func (r *driverReconcile) canSnapshotMetadataBeDeployed() bool {
	...
	return (r.isRdbDriver() && snapshotMetadataServiceCRDInstalled && enableSnapshotMetadata && snPolicy != csiv1a1.NoneSnapshotPolicy)
}
...
func (r *driverReconcile) createTLSSecret(caCert *x509.Certificate, certBytes, keyBytes []byte) error {
	...
	tlsSecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      utils.SnapshotMetadataTLSSecretName,
...
func (r *driverReconcile) createCertsAndSecrets() (*corev1.Secret, error) {
	...
	tlsSecret := &corev1.Secret{}
	tlsSecret.Name = utils.SnapshotMetadataTLSSecretName
...
func (r *driverReconcile) createResourcesForSnapshotMetadata() error {
	...
	if r.snapshotMetadataCertExpiry.Before(time.Now().Add(-24 * time.Hour)) {
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					Volumes: utils.Call(func() []corev1.Volume {
						...
						if r.canSnapshotMetadataBeDeployed() {
							volumes = append(volumes, utils.SnapshotMetadataServerCertsVolume)
						}
						if r.driver.Spec.Encryption != nil {
							volumes = append(
								volumes,

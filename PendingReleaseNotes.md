...
- Added example PodMonitor manifests (`deploy/examples/podmonitor/`) and usage docs (`docs/design/podmonitor.md`) for scraping CSI controller-plugin sidecar metrics with the Prometheus Operator. No operator API changes; deploy via `kubectl apply` or the drivers chart `extraDeploy` value.
## NOTE
